...
\begin{theorem}\footnote{Все пределы здесь рассматриваются при \(n \to \infty\), что опускается ради читаемости текста.}
...
\subsection{\color{red} Теорема о функциях ограниченной вариации}
...
%\begin{theorem}
%	Число \(e^2\) (а значит, и \(e\) тоже) иррационально.
%\end{theorem}
%\begin{proof}
%	Предположим обратное. Пусть \(e^2 = 2k/n\) (возможно, эта дробь сократима, нам просто удобно иметь числитель чётным). Тогда \[
%		en = 2k \, e^{-1} \iff en (2k - 1)! = (2k)! \, e^{-1}.
%	\]
%	Далее мы планируем сделать следующее: показать, что левая часть чуть больше некоторого целого числа (не более, чем на одну треть), а правая --- чуть меньше некоторого целого числа (тоже не более, чем на треть), и, соответственно, равенства между ними быть не может.
%
%	Разложим экспоненту в ряд Маклорена с остатком в форме Лагранжа и подставим единицу: \[
%		e = 1 + \frac{1}{1!} + \frac{1}{2!} + \ldots + \frac{1}{n!} + \frac{e^c}{(n+1)!}, \ \text{где} \ c \in [0, 1].
%	\]  
%	Теперь левую часть равенства можно записать в виде
%	\begin{multline*}
%		\left(1 + \frac{1}{1!} + \frac{1}{2!} + \ldots + \frac{1}{(2k-1)!} + \frac{1}{(2k)!} + \ldots \right) \cdot n \cdot (2k - 1)! = \\
%		= \left(1 + \frac{1}{1!} + \frac{1}{2!} + \ldots + \frac{1}{(2k-1)!} \right) \cdot n \cdot (2k - 1)! + \left(\frac{1}{2k} + \frac{1}{2k \, (2k+1)} + \ldots \right) \cdot n.
%	\end{multline*}
%	Первое слагаемое --- целое число, а во втором заменим все знаменатели на 		
%\end{proof}
...
\begin{lemma}
	Пусть \((A), (B)\) --- строго положительные ряды, причём, НСНМ, \(\dfrac{a_{n+1}}{a_n} < \dfrac{b_{n+1}}{b_n}\). Тогда:
	\begin{enumerate}
		\item \((B)\) сходится \(\Rightarrow\) \((A)\) сходится,
		\item \((A)\) расходится \(\Rightarrow\) \((B)\) расходится.
	\end{enumerate}
...
\begin{proof}
	...
	\begin{figure}[h!]
		\label{qnum}
		\centering{\includegraphics[width=0.5 \linewidth]{qnum.eps}}
...
\subsection{\color{red} Теорема о двойных и повторных пределах}
