...
\begin{remark}
	...
	\begin{enumerate}
		\item $\delta$ монотонна: $A \subset B \Rightarrow \delta (A) \leqslant \delta (B)$,
...
\begin{proof}
	...
	\begin{enumerate}
		\item Поскольку \hbox{$B = A \sqcup (B \setminus A)$}, то \hbox{$\delta (B) = \delta (A) + \delta (B \setminus A) \geqslant \delta (A)$},
...
\begin{definition}
	...
	\begin{enumerate}
		\item Монотонность: если $A \subset B \hbox{, то } \delta (A) \leqslant \delta (B)$,
		\item Нормировка: $\hbox{$\delta ([a, b] \times [c, d])$} = (b - a) (d - c)$,
